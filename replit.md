...
- **Backend:** Express.js handles all API routes and business logic, which was fully ported from Supabase Edge Functions. Drizzle ORM manages interactions with the external PostgreSQL database. Connection pool: 20 max connections, 30s statement timeout, 5s connection timeout. Database indexes are auto-created on startup (reviews: cabinet_id+status, cabinet_id+created_date, user_id, wb_id+user_id+cabinet_id; partial index on status+updated_at for archival).
...
- **Business Logic:** Centralized in `server/functions.ts`, including review synchronization, chat synchronization, reply generation and sending, AI assistant for analytics, and payment processing. Sync uses per-cabinet locking (prevents duplicate syncs), parallel cabinet processing (concurrency=2), bulk DB lookups for existing reviews (1 query instead of N), and batched AI generation (5 concurrent). Archival runs in batches of 1000 to avoid table locks.
